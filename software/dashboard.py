...
COLORS = {
    ...
    'blue': '#89C4E1', 'pink': '#FF3D8A', 'lavender': '#B39DDB',
    'white': '#F0F0F0', 'gray': '#8A8A9A', 'gray_dim': '#555566',
...
/* ─────────────────────────────────────────────────────────────────────────────
   SIDEBAR — minimal overrides, let the dark theme handle text color
   ───────────────────────────────────────────────────────────────────────────── */
...
/* Force all sidebar text to white — works regardless of theme */
...
/* ── Sidebar NAV BUTTONS (must come AFTER the global button rule below) ──── */
/* This block is intentionally placed at the bottom to override the gradient */
...
/* ── SIDEBAR BUTTONS — override gradient, placed AFTER global rule ── */
...
/* Active nav button (type=primary in sidebar) */
...
[data-testid="stSidebar"] button[kind="primary"]:hover {
    background: rgba(137,196,225,0.18) !important;
    opacity: 1 !important;
}
...
def podium_card(col, rank, name, total, runs, top_speed):
    ...
    <div style="background:{c['bg']};border:1px solid {c['border']};
        ...
        <div style="position:absolute;top:12px;right:14px;
                    ...
                    text-transform:uppercase;color:{c['bc']};
                    background:{c['bc']}18;border:1px solid {c['bc']}44;
        ...
        <div style="font-family:'Bebas Neue',sans-serif;font-size:1.8rem;
                    letter-spacing:0.1em;color:{c['nc']};margin-bottom:6px;">{name.upper()}</div>
        ...
        <div style="font-family:'DM Sans',sans-serif;font-size:0.75rem;
                    color:#555566;">{runs} runs &nbsp;·&nbsp; {top_speed:.1f} mph top speed</div>
    </div>""", unsafe_allow_html=True)
...
def render_footer():
    ...
    <div style="margin-top:48px;padding:20px 0;border-top:1px solid #1E1E2E;
        ...
        <span style="font-family:'JetBrains Mono',monospace;font-size:0.7rem;color:#555566;">v2.1</span>
    </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# SIDEBAR — session_state navigation via buttons
# ══════════════════════════════════════════════════════════════════════════════
NAV_ITEMS = [
    ("🏆", "LEADERBOARD"),
    ("👤", "ATHLETE PROFILES"),
...
with st.sidebar:
    ...
    <div style="padding:16px 0 16px;">
        ...
        <div style="font-family:'DM Sans',sans-serif;font-size:0.62rem;letter-spacing:0.18em;
                    text-transform:uppercase;color:#555566;margin-top:2px;">
            acceleration starts here</div>
    </div>
    <div style="height:1px;background:linear-gradient(90deg,#89C4E1,#FF3D8A);
                margin-bottom:16px;opacity:0.35;"></div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="background:#0A0A0F;border:1px solid #1E1E2E;border-radius:10px;padding:14px 16px;">
        <div style="font-family:'DM Sans',sans-serif;font-size:0.6rem;letter-spacing:0.14em;
                    text-transform:uppercase;color:#444455;margin-bottom:10px;">Live snapshot</div>
...
if page == "LEADERBOARD":
    ...
    fig = go.Figure(go.Bar(
        x=lb_sorted['athlete'], y=lb_sorted['best_total'],
    ...
    for split, label, color in [
        ...
        fig2.add_trace(go.Bar(
            y=lb['athlete'], x=lb[split], name=label,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    selected = st.selectbox("Select athlete", all_athletes,
                            label_visibility="collapsed", key="profile_select")
    athlete_data = load_athlete_runs(selected)
    profile = load_athlete_profile(selected)

    if athlete_data.empty:
        empty_state("NO RUNS FOR THIS ATHLETE")
        render_footer()
        st.stop()
    ...
    <div style="background:linear-gradient(135deg,#12121A 0%,#1A1A26 100%);
        ...
        <div style="width:72px;height:72px;border-radius:50%;
            ...
            <span style="font-family:'Bebas Neue',sans-serif;font-size:2.2rem;color:#0A0A0F;">
                {selected[0].upper()}</span>
        </div>
        <div style="flex:1;min-width:160px;">
            <div style="font-family:'Bebas Neue',sans-serif;font-size:2.4rem;
                        letter-spacing:0.08em;color:#F0F0F0;line-height:1;">{selected.upper()}</div>
            <div style="font-family:'DM Sans',sans-serif;font-size:0.75rem;
                        color:#555566;margin-top:4px;">
    ...
    fig.add_trace(go.Scatter(x=df_asc['run_num'], y=df_asc['total'],
        ...
        line=dict(color='#89C4E1', width=2), marker=dict(size=5, color='#89C4E1'),
    ...
    field_avg = [all_runs['split_0_10'].mean(), all_runs['split_10_30'].mean(), all_runs['split_30_60'].mean()]
    ath_avg   = [athlete_data['split_0_10'].mean(), athlete_data['split_10_30'].mean(), athlete_data['split_30_60'].mean()]
    fig2 = go.Figure()
    fig2.add_trace(go.Scatterpolar(r=field_avg+[field_avg[0]], theta=cats+[cats[0]],
        fill='toself', name='Field avg', line=dict(color='#555566', width=1),
        fillcolor='rgba(85,85,102,0.15)'))
    fig2.add_trace(go.Scatterpolar(r=ath_avg+[ath_avg[0]], theta=cats+[cats[0]],
        fill='toself', name=selected, line=dict(color='#89C4E1', width=2),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    athlete_b = c2.selectbox("Athlete B", all_athletes, index=min(1, len(all_athletes)-1), key="compare_b")
    da, db_ = load_athlete_runs(athlete_a), load_athlete_runs(athlete_b)

    st.markdown('<div style="height:8px"></div>', unsafe_allow_html=True)
    for label, col, fmt, lib in [
        ...
        ca, cb = ('#FFD700' if wa else '#555566'), ('#FFD700' if wb else '#555566')
        l, mid, r = st.columns([2, 1, 2])
        l.markdown(f"""<div style="text-align:right;padding:10px 16px;background:#12121A;
            border:1px solid {'#FFD700' if wa else '#1E1E2E'};border-radius:10px;margin-bottom:6px;">
            <span style="font-family:'JetBrains Mono',monospace;font-size:1.3rem;color:{ca};">{fmt.format(va)}</span>
            {'🏆' if wa else ''}</div>""", unsafe_allow_html=True)
        mid.markdown(f"""<div style="text-align:center;padding:14px 0;">
            <div style="font-family:'DM Sans',sans-serif;font-size:0.62rem;letter-spacing:0.12em;
                        text-transform:uppercase;color:#2A2A3E;">{label}</div></div>""",
            unsafe_allow_html=True)
        r.markdown(f"""<div style="text-align:left;padding:10px 16px;background:#12121A;
            border:1px solid {'#FFD700' if wb else '#1E1E2E'};border-radius:10px;margin-bottom:6px;">
            {'🏆' if wb else ''}<span style="font-family:'JetBrains Mono',monospace;font-size:1.3rem;color:{cb};">{fmt.format(vb)}</span>
            </div>""", unsafe_allow_html=True)
    ...
    for df_x, name, color, fc in [
        ...
        fig2.add_trace(go.Scatterpolar(r=v+[v[0]], theta=cats+[cats[0]], fill='toself',
            name=name, line=dict(color=color, width=2), fillcolor=fc))
    fig2.update_layout(
        polar=dict(bgcolor='#12121A',
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if len(df_p) >= 5:
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin:16px 0 24px;">
            <div style="background:#12121A;border:1px solid #1DDB8B44;border-left:3px solid #1DDB8B;border-radius:10px;padding:14px 20px;">
                <div style="font-family:'JetBrains Mono',monospace;font-size:1.6rem;color:#1DDB8B;">-{delta:.3f}s</div>
                <div style="font-family:'DM Sans',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#555566;margin-top:4px;">Time improved (first 5 vs last 5)</div>
            </div>
            <div style="background:#12121A;border:1px solid #89C4E144;border-left:3px solid #89C4E1;border-radius:10px;padding:14px 20px;">
                <div style="font-family:'JetBrains Mono',monospace;font-size:1.6rem;color:#89C4E1;">{pct:.1f}%</div>
                <div style="font-family:'DM Sans',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#555566;margin-top:4px;">Percentage faster</div>
            </div>
            <div style="background:#12121A;border:1px solid #FF3D8A44;border-left:3px solid #FF3D8A;border-radius:10px;padding:14px 20px;">
                <div style="font-family:'JetBrains Mono',monospace;font-size:1.6rem;color:#FF3D8A;">{df_p['top_speed'].max():.1f} mph</div>
                <div style="font-family:'DM Sans',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#555566;margin-top:4px;">Peak top speed</div>
            </div>
        </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("log_run"):
        ...
        if st.form_submit_button("LOG RUN", use_container_width=True) and all_athletes:
            total = s1 + s2 + s3
